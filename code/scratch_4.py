...
def fillinetree(table,Tbl,Worktbl):
    label = Tbl[matching[1]].notnull().astype(int).to_frame()
    ...
    if sum(label_train.values) != 0:
        ...
        print(str(table['iter']) + ": "+str(table['Bcr_test'] ))
    return table
...
t9.to_csv("C:\\Users\cocol\Desktop\memoire\Jéjé_work\metaparam"+str(matching[1])+".csv")