...
for exo in matching:
    ...
    if sum(label_train.values) != 0:
        ...
        clf = tree.DecisionTreeClassifier(max_depth=param['max_depth'].values[0],criterion=param['criterion'].values[0],min_samples_split=param['min_samples_split'].values[0],min_impurity_decrease=param['min_impurity_decrease'].values[0])
...
clf2 = pickle.load(open("modeltoexport\\modelfor_1006_frequency.sav", 'rb'))
test_pred = clf.predict(test)

balanced_accuracy_score(label_test, test_pred)
